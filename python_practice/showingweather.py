...
class naverWeather():
    ...
    def __init__(self, area,number):
        ...
        self.addr = "http://weather.naver.com/rgn/cityWetrCity.nhn?cityRgnCd=CT" + number
        self.result = None
        self.area = area
...
def main():
    ...
    keep_going = True
    myarea = input("지역을 입력해 주세요: ")
    while keep_going:
        if myarea in map_cityNum:
            mynumber = map_cityNum[myarea]
            keep_going = False

        else:
            myarea = input("지역을 다시 입력해주세요: ")

    getting_weather_info = naverWeather(myarea,mynumber)
    print(getting_weather_info.search())
